...
const PayrollBanner = ({ empleado = '—', salario = 0 }) => (
  <div className="relative bg-gradient-to-br from-blue-700 via-blue-600 to-slate-800 rounded-2xl p-8 mb-6 overflow-hidden shadow-xl">
    ...
    <div className="absolute -bottom-8 right-10 w-28 h-28 bg-emerald-400/10 rounded-full pointer-events-none" />

    <div className="relative flex items-center justify-between gap-6">
      <div>
        <div className="flex items-center gap-2 mb-3">
          <TbReceipt className="text-emerald-400 text-xl" />
      ...
      <div className="flex flex-col gap-3 flex-shrink-0">
        <button className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white border border-white/20 rounded-xl px-5 py-2.5 text-sm font-medium transition-all backdrop-blur-sm cursor-pointer">
          <FiEye /> Ver detalle
        </button>
        <button className="flex items-center gap-2 bg-emerald-400 hover:bg-emerald-300 text-slate-900 rounded-xl px-5 py-2.5 text-sm font-bold transition-all shadow-lg shadow-emerald-400/30 cursor-pointer">
